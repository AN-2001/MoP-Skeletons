...
#define USER_FONT_STR_MALLOC(n)		IritMalloc(sizeof(UserFontChar) * (n) + 1)
#define USER_FONT_STR_FREE(Str)		IritFree(Str)
#define USER_FONT_STR_CPY(DStr, SStr)	strcpy(DStr, SStr)
#define USER_FONT_STR_NCPY(DStr, SStr, n) strîcpy(DStr, SStr, n)
#define USER_FONT_STR_CAT(DStr, SStr)	strcat(DStr, SStr)
#define USER_FONT_STR_DUP(Str)		IritStrdup(Str)
...
typedef struct UserSpkSolvingStatus {     /* Solving status and statistics. */
...
typedef struct UserGr2DSwpParamStruct {
    ...
    IPObjectStruct *Crv1;   /* Input gear to conjugate, or overall shape of */
       /* gear to arrange teeth on, or centrode (return argument), or       */
    ...
    IPObjectStruct *CrvSrf; /* Gear obtained after arranging teeth (return  */
          /* argument), or planar surface within which to compute centrode. */
...
typedef struct UserTrussSpherePackParamsStruct {
    ...
    UserSpkAlgorithm SpherePackAlg;
...
IPObjectStruct *UserMJMicroJoinMain(const TrivTVStruct **BaseTVs,
				    const IPObjectStruct **BaseTileGeoms,
				    int NumBaseTileSets,
				    VMdlBoolOpType OpType);
IPObjectStruct* UserMJMicroJoinSimple(UserMJTileStrctStruct *MJTA,
				      UserMJTileStrctStruct *MJTB,
				      VMdlBoolOpType OpType);
void UserMJVMdlBooleanCallBack(const VMdlVModelStruct *VMdl1,
			       const VMdlVModelStruct *VMdl2,
			       VMdlVModelStruct *ResVMdl,
			       VMdlBoolOpType BoolOp,
			       void *AuxData);
IPObjectStruct *UserMJMicroJoinSynthesize(const VMdlVModelStruct *VMdl,
					  CagdRType NrmScale,
					  CagdRType NrmBlendingRatio,
					  CagdRType SaddleRatio);
IPObjectStruct *UserMJGenSweepTV(const CagdSrfStruct *SrfIn,
				 IrtVecType DirIn,
				 const UserMJJoinInterSrfStruct *JISrf,
				 CagdRType NormalScale);
IPObjectStruct *UserMJGenSweepTV2(const CagdSrfStruct *SrfIn,
				  const CagdSrfStruct *SrfOut,
				  IrtVecType DirIn,
				  IrtVecType DirOut,
				  CagdRType NormalScale,
				  CagdBType BMinTwist);

/* Create a bifurcation tile connecting one inlet to two outlets. */
IPObjectStruct *UserMJGenBifurcation1To2Tile(const CagdSrfStruct *SrfIn,
					     const CagdSrfStruct *SrfOut1,
					     const CagdSrfStruct *SrfOut2,
					     IrtVecType DirIn,
					     IrtVecType DirOut1,
					     IrtVecType DirOut2,
					     CagdRType SaddleRatio);
...
/* Tiles in 2D - semiregular. */
IPObjectStruct *UserTile2DSemi44333(CagdRType TileSize);
IPObjectStruct *UserTile2DSemi6363(CagdRType TileSize);
IPObjectStruct *UserTile2DSemi848(CagdRType TileSize);
IPObjectStruct *UserTile2DSemi4346(CagdRType TileSize);
IPObjectStruct *UserTile2DSemi33336(CagdRType TileSize);
IPObjectStruct *UserTile2DSemi43433(CagdRType TileSize);
IPObjectStruct *UserTile2DSemi12312(CagdRType TileSize);
IPObjectStruct *UserTile2DSemi1246(CagdRType TileSize);
...
IPObjectStruct *UserWDDitherRGB(IritImgPrcssImgStruct *Image0,
...
IPObjectStruct *UserWDDitherBW(IritImgPrcssImgStruct *Image0,
			       IritImgPrcssImgStruct *Image1,
			       int NumberOfLines,
			       int NumberOfPins,
			       float LineIntensity,
...
/* Run a given sphere packing algorithm on a given border within a given time */
/* limit (budjet).							      */
IPObjectStruct *UserSpkSolveForBorder(UserSpkBorderStructPtr Border,
                                     IrtRType SphereRadius,
                                     IrtRType TimeLimitInSeconds, 
                                     UserSpkAlgorithm Algorithm);
